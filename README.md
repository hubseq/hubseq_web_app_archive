...
A bioinformatics hub for all of your sequencing data
...
```bash
mkvirtualenv hubseq_web_app -p python3
pip install -r requirements.txt
```

## Set up database

Create a database named `hubseq_web_app`.

```
createdb hubseq_web_app
```
...
```bash
celery -A hubseq_web_app worker -l INFO
```
